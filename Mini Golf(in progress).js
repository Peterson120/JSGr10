...
https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.clipartkey.com%2Fview%2FoRTRix_info-icon-characters[6]-png-clipart-png-download-info%2F&psig=AOvVaw0off1aXbBykG2xLHwQxM83&ust=1623719124487000&source=images&cd=vfe&ved=0CAIQjRxqFwoTCJCf5aD3lfECFQAAAAAdAAAAABAD
...
https://www.google.com/imgres?imgurl=https%3A%2F%2Fwww.searchpng.com%2Fwp-content%2Fuploads%2F2019%2F02%2FReload-characters[6]-Icon-PNG.png&imgrefurl=https%3A%2F%2Fwww.searchpng.com%2F2019%2F02%2F12%2Freload-characters[6]-icon-transparent-png-free-download%2F&tbnid=MHgqKso8zXccGM&vet=12ahUKEwj3jLePjJjxAhWQdqwKHdEdDc4QMygAegUIARDCAQ..i&docid=zv4yxmcABlecbM&w=2048&h=2048&q=reload%20icon%20no%20background%20characters[6]&ved=2ahUKEwj3jLePjJjxAhWQdqwKHdEdDc4QMygAegUIARDCAQ
...
const characters = [new WebImage("https://codehs.com/uploads/a16cebb278e0cd14ada0bc1ae614d507"), 
                    new WebImage("https://codehs.com/uploads/ad822dfab14db07c84a839f9538b2526"),
                    new WebImage("https://codehs.com/uploads/95d4d9e344a7fd040a524c0ee43d6947"),
                    new WebImage("https://codehs.com/uploads/239e140d4481de583fe7442e51497a98"),
                    new WebImage("https://codehs.com/uploads/846b269bf0a76f60f460a5dbcf5ebd53"),
                    new WebImage("https://codehs.com/uploads/8f4b4258cac366d43d0dc45e7cd3f11e"),
                    new WebImage("https://codehs.com/uploads/e6c475f8f3b16705ed55af645f2c0855"),
                    new WebImage("https://codehs.com/uploads/a7a483be8517359031713a9616a31dda")];
...
function items(e) {
	...
	//adds items if instructions are clicked
        ...
        let Y = addInstructionText();
        
        start_button.setPosition(getWidth()/2 - start_button.getWidth()/2, Y + 40);
...
function characterSelection() {
    ...
    characters[0].setSize(50,50);
    characters[0].setPosition(10,rectangle.getY() + rectangle.getHeight()/5);
	add(characters[0]);
    for (let i = 1; i < characters.length; i++) {
        characters[i].setSize(50,50);
        characters[i].setPosition(characters[i-1].getX() + characters[i-1].getWidth() + 10, rectangle.getY() + rectangle.getHeight()/5);
        add(characters[i]);
    }
}
...
function character(e) {
    ...
    if (character_num == null) {
        switch(elem) {
            case characters[0]:
                characters[0].setPosition(getWidth()/60,getHeight()/60);
    	        character_num = 0;
    	        break;
            case characters[1]:
                characters[1].setPosition(getWidth()/60 + 10,getHeight()/60);
                character_num = 1;
                break;
            case characters[2]:
                characters[2].setPosition(getWidth()/60,getHeight()/60);
                character_num = 2;
                break;
            case characters[3]:
                characters[3].setPosition(getWidth()/60,getHeight()/60);
                character_num = 3;
                break;
            case characters[4]:
                characters[4].setPosition(getWidth()/60 + 10,getHeight()/60);
                character_num = 4;
                break;
            case characters[5]:
                characters[5].setPosition(getWidth()/60 + 10,getHeight()/60);
                character_num = 5;
                break;
            case characters[6]:
                characters[6].setPosition(getWidth()/60 + 10,getHeight()/60);
                character_num = 6;
                break;
            case characters[7]:
                characters[7].setPosition(getWidth()/60 + 10,getHeight()/60);
                character_num = 7;
                break;
            default:
                return;
...
function startClick() {
    ...
    add(characters[character_num]);
    characters[character_num].setPosition(getWidth()/60 + 10,getHeight()/60);
...
//text
	...
	you.setPosition(characters[character_num].getX() + characters[character_num].getWidth()/2 - you.getWidth()/2,characters[character_num].getY() + characters[character_num].getHeight() + you.getHeight());
	you.setColor("#FFD700");
	add(you);
	
//sets score and adds white background
	score_circle = new Circle(20);
	score_circle.setPosition(characters[character_num].getX() + characters[character_num].getWidth() + score_circle.getWidth()/2+10,characters[character_num].getY() + characters[character_num].getHeight()/2);
...
function slowBall() {
    ...
    if (level >= 2) {
        ...
        if (ball.getRadius()+water.getRadius()>=distance) {
            ...
            x -= x_decrease;
            y -= y_decrease;
        } else if (xball+radius>=ramp.x
            &&xball-radius<=ramp.x+ramp.getWidth()
            &&yball+radius>=ramp.y
            &&yball-radius<=ramp.y+ramp.getHeight()) {
            y_decrease = y+0.1;
            y = y_decrease;
        } else {
            x *= 0.99;
...
function click(e) {
    ...
    //checks if reload icon is clicked
	    ...
	    //moves ball
	    mouseDragMethod(move);
	    //checks if characters[6] instruction button is clicked
	    mouseClickMethod(click);
    }
}
...
//counts up and sets new score position and text
    if (elem == how_to_play) {
	    ...
	    //checks if characters[6] instruction button is clicked
	    mouseClickMethod(click);
    } else if (elem.getColor() == Color.green || elem.getColor() == Color.white || elem.getColor() == Color.cyan || elem.getColor() == Color.black || elem == ramp && elem != play_again && elem != play_again_button && elem != next_level && elem != next_level_button) {
        count++;
        score.setText(count);
        score.setPosition(score_circle.getX()-score.getWidth()/2,score_circle.getY()+score.getHeight()/2-3);
        setTimer(draw,1);
...
function collisionDetection() {
    for(var i = 0; i < vertical.length; i++) {
        verticalRect(vertical[i]);
    }
    for(var i = 0; i < horizontal.length; i++) {
        horizontalRect(horizontal[i]);
    }
...
function horizontalRect(rect) {
    var xball = ball.x;
    var yball = ball.y;
    var radius = ball.getRadius();
    
    if (xball+radius>=rect.getX()
        &&xball-radius<=rect.getX()+rect.getWidth()
        &&yball+radius>=rect.getY()
        &&yball+radius<rect.getY()+rect.getHeight()
        ||xball+radius>=rect.getX()
        &&xball-radius<=rect.getX()+rect.getWidth()
        &&yball-radius<=rect.getY()+rect.getHeight()
        &&yball-radius>rect.getY()) {
        y=-y;
    } else if (xball+radius>=rect.getX()
        &&xball+radius<rect.getX()+rect.getWidth()
        &&yball+radius>=rect.getY()
        &&yball-radius<=rect.getY()+rect.getHeight()
        ||xball-radius>rect.getX()
...
function verticalRect(rect) {
    ...
    if (xball+radius>=rect.getX()
        ...
        &&yball-radius<=rect.getY()+rect.getHeight()) {
        x=-x;
        ball.setPosition(rect.x,ball.y);
    } else if (xball-radius<=rect.getX()+rect.getWidth()
        &&xball-radius>rect.getX()
        &&yball+radius>=rect.getY()
        &&yball-radius<=rect.getY()+rect.getHeight()) {
        x=-x;
        ball.setPosition(rect.x+rect.getWidth(),ball.y);
    } else if (yball+radius>=rect.getY()
        &&yball+radius<rect.getY()+rect.getHeight()
        &&xball+radius>=rect.getX()
        &&xball-radius<=rect.getX()+rect.getWidth()) {
        y=-y;
        ball.setPosition(ball.x,rect.y);
        } else if (yball-radius<=rect.getY()+rect.getHeight()
        &&yball-radius>rect.getY()
        &&xball+radius>=rect.getX()
        &&xball-radius<=rect.getX()+rect.getWidth()) {
        y=-y;
...
function obstacles() {
    ...
    if (level >= 2) {
	    ...
	    ramp = new Rectangle(Randomizer.nextInt(100,200),Randomizer.nextInt(50,75));
	    var color = new Color(1,121,111);
	    ramp.setColor(color);
	    ramp.setPosition(Randomizer.nextInt(getWidth()/2,getWidth()-ramp.getWidth()),Randomizer.nextInt(getHeight()/2,ball.getY()+ball.getRadius()+ramp.getHeight()));
	    add(ramp);
	    
	    var line = new Line(ramp.getX()+ramp.getWidth()/2,ramp.getY()+ramp.getHeight()/3,ramp.getX()+ramp.getWidth()/2,ramp.getY()+ramp.getHeight()-ramp.getHeight()/4);
	    add(line);
	    var line2 = new Line(ramp.getX()+ramp.getWidth()/4,ramp.getY()+ramp.getHeight()/2,ramp.getX()+ramp.getWidth()/2,ramp.getY()+ramp.getHeight()-ramp.getHeight()/4);
	    add(line2);
	    var line3 = new Line(ramp.getX()+ramp.getWidth()-ramp.getWidth()/4,ramp.getY()+ramp.getHeight()/2,ramp.getX()+ramp.getWidth()/2,ramp.getY()+ramp.getHeight()-ramp.getHeight()/4);
	    add(line3);
	}
	...
	if (cheats != "No Walls") {
	    if (level >= 2) {
	        for(var i = 0; i < horizontal.length; i++) {
            	...
            	horizontal[i].setPosition(Randomizer.nextInt(getWidth()/2,ramp.getX()-horizontal[i].getWidth()-20),Randomizer.nextInt(getHeight()/3, getHeight()-getHeight()/3));
            	add(horizontal[i]);
	        }
            ...
            for(var i = 0; i < vertical.length; i++) {
            	vertical[i].setColor(Color.white);
            	vertical[i].setPosition(Randomizer.nextInt(getWidth()/3,ball.getX()-ball.getRadius()*2-vertical[i].getWidth()),Randomizer.nextInt(vertical[i].getHeight()+horizontal_walls.getHeight(),getHeight() - ball.getRadius()*2 - vertical[i].getHeight()-horizontal_walls.getHeight()));
            	add(vertical[i]);
            }
	    }
	}
...
//adds and removes items when characters[6] how to play button is clicked
function info() {
    for(var i = 0; i < horizontal.length; i++) {
        remove(vertical[i]);
        remove(horizontal[i]);
...
function addInstructionText() {
    const arr = ["9pt Arial", 
        ...
        "Try to get the ball in the hole in as little hits as possible.",
        "There are three levels all with a different feature",
        "Do not touch the ball until it is fully stopped",
        "Level too hard? Just click the reload icon."];
    rectangle = new Rectangle(getWidth() - 100, getHeight()/3 + 40);
    rectangle.setPosition(getWidth()/2-rectangle.getWidth()/2,getHeight()/2-rectangle.getHeight()/2);
    rectangle.setColor(Color.white);
    add(rectangle);
    
    instructions = new Text(arr[1]);
    instructions.setPosition(getWidth()/2 - instructions.getWidth()/2,rectangle.getY() + instructions.getHeight());
    add(instructions);
    
    instruction1 = new Text(arr[2], arr[0]);
    instruction1.setPosition(getWidth()/2 - instruction1.getWidth()/2, instructions.getY() + instruction1.getHeight()*2);
    add(instruction1);
    
    instruction_cont = new Text(arr[3], arr[0]);
